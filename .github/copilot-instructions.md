...
Prerequisites: **Conan >= 2.0** and **CMake >= 3.25**.

1. **Install dependencies with Conan**, then **strict build** via explicit
  toolchain configure:
   ...
   example, `conan-release` vs `conan-default`), so CI and automation should
   not depend on preset names.
...
- `cmake_layout` places generated files and build output under `build/<Config>`
  ...
  against the project root (`conan install ..`). Configure with an explicit
  toolchain path (for example
  `-DCMAKE_TOOLCHAIN_FILE=build/Release/generators/conan_toolchain.cmake`) for
  portable CI and automation.
